...
class Hit:
    def __init__(self,hitindex,hitid,process_parent_name,process_name,process_command_line,user_name,host_name,timestamp,time,\
                 destination_ip,destination_port,source_ip,source_port,source_user_name,url_domain,url_path,url_full,\
                 rule_category,dns_question_name,dns_resolvedip,event_dataset,event_action,event_type):
                ...
                for index in self.index:
                    ...
                    for hit in res['hits']['hits']:
                        ...
                        if 'process' in hit['_source']:
                            ...
                            if 'command_line' in hit['_source']['process']:
                                process_command_line = hit['_source']['process']['command_line']
                        #destination fields
                        ...
                        if 'event' in hit['_source']:
                            ...
                            if 'type' in hit['_source']['event']:
                                event_type = hit['_source']['event']['type']
                            if 'dataset' in hit['_source']['event']:
                                event_dataset = hit['_source']['event']['dataset']
                        #url fields
                        ...
                        hits.append(Hit(hitindex,hitid,process_parent_name,process_name,process_command_line,user_name,host_name,\
                        timestamp,time,destination_ip,destination_port,source_ip,source_port,source_user_name,\
                            url_domain,url_path,url_full,rule_category,dns_question_name,dns_resolvedip,event_dataset,event_action,event_type))
